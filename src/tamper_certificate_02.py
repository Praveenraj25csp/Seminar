#!/usr/bin/env python3
import json, base64, hashlib, sys, re, os, tempfile
...
def tamper(pdf_in: str, pdf_out: str, key: str, new_value: str):
    ...
    inplace = False
    try:
        if pdf_out_path.resolve() == pdf_in_path.resolve():
            inplace = True
    except Exception:
        pass

    if inplace:
        fd, tmp_path = tempfile.mkstemp(suffix=".pdf", dir=str(pdf_in_path.parent))
        os.close(fd)
        with open(tmp_path, "wb") as f:
            w.write(f)
        os.replace(tmp_path, str(pdf_in_path))
        saved_path = pdf_in_path
    else:
        pdf_out_path.parent.mkdir(parents=True, exist_ok=True)
        with open(pdf_out_path, "wb") as f:
            w.write(f)
        saved_path = pdf_out_path
...
def main():
    cfg = load_config(Path("../inputs/certificate_tamper_input.json"))
    pdf = cfg.get("pdf")
    out = cfg.get("out")
    payload_change = cfg.get("payload_change")
    if not pdf or not out or not payload_change:
        print("inputs must include: pdf, out, payload_change", file=sys.stderr); sys.exit(2)
    if "=" not in payload_change:
        print("payload_change must be key=value", file=sys.stderr); sys.exit(2)
    k, v = payload_change.split("=", 1)
    tamper(pdf, out, k, v)
